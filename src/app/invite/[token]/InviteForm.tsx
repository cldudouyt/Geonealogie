      ...
      <div style={cardStyle}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{
            ...
            {isReset ? 'Réinitialiser votre mot de passe' : 'Bienvenue sur Géonéalogie'}
          </h1>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {!isReset && (
            <div>
              ...
              <input
                id="name" name="name" type="text" required={!isReset} autoFocus
                placeholder="Marie"
          ...
          <div>
            <label htmlFor="password" style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#3a4038', marginBottom: 6 }}>
              {isReset ? 'Nouveau mot de passe' : 'Mot de passe'}
            </label>
            <input
              id="password" name="password" type="password" required autoFocus={isReset}
          ...
          <button type="submit" disabled={isPending} style={{ ...btnStyle, opacity: isPending ? 0.65 : 1 }}>
            {isPending
              ? (isReset ? 'Enregistrement…' : 'Activation…')
              : (isReset ? 'Enregistrer le nouveau mot de passe' : 'Activer mon compte')}
          </button>
        </form>
      </div>
    </div>
